  ...
  return (
    <div className="space-y-4">
      ...
      {(profile.data || auth.me) ? <div className="grid gap-4 lg:grid-cols-[minmax(0,1.15fr)_minmax(0,0.85fr)]">
        <Card>
          <CardHeader><CardTitle className="flex items-center gap-2 text-base"><GraduationCap className="h-4 w-4 text-primary" />基础资料</CardTitle></CardHeader>
          <CardContent className="grid gap-x-6 gap-y-0 sm:grid-cols-2">
            {details.map((item) => <div key={item.label} className="border-b py-3 last:border-b-0 sm:last:border-b"><p className="text-xs font-medium text-muted-foreground">{item.label}</p><p className="mt-1 break-words text-sm font-semibold">{item.value}</p></div>)}
          </CardContent>
        </Card>
        <Card>
...
function StatusRow({ icon, label, value }: { icon: ReactNode; label: string; value: string }) {
  return <div className="flex items-center justify-between gap-4 rounded-lg bg-muted/45 px-3 py-2.5"><span className="flex items-center gap-2 text-muted-foreground">{icon}{label}</span><span className="text-right font-semibold">{value}</span></div>;
}
